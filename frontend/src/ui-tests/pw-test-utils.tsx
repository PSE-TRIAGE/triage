...
import type { AuthService, User } from "@/api/services/auth.service";
...
import type { AdminUsersService, AdminUser } from "@/api/services/admin-users.service";
import type { AdminFormFieldService, FormField } from "@/api/services/admin-formfield.service";
...
import type { MutantsService, MutantOverview } from "@/api/services/mutants.service";
...
export function createMockServices(overrides: Partial<Services> = {}): Services {
  return {
    authService: {
      ...
      logout: noop,
      me: async () => ({ id: "1", username: "testuser", isAdmin: false, isActive: true }),
    } as AuthService,
    projectsService: {
      listProjects: async () => [],
    ...
    userService: {
      changeUsername: async () => ({ id: "1", username: "new", isAdmin: false, isActive: true }),
      changePassword: noop,
    ...
    adminFormFieldService: {
      ...
      createFormField: async () => ({ id: 1, projectId: 1, label: "Test", type: "text", isRequired: false, position: 0 }),
      updateFormField: async () => ({ id: 1, projectId: 1, label: "Updated", type: "text", isRequired: false, position: 0 }),
      deleteFormField: noop,
      reorderFormFields: noop,
    } as unknown as AdminFormFieldService,
    exportService: {
      getExportPreview: async () => ({ project_id: 1, project_name: "Test", stats: {}, sample_entries: [] }),
      downloadExport: noop,
      getExportData: async () => ({ project_id: 1, project_name: "Test", exported_at: "", stats: {}, ratings: [] }),
    } as unknown as ExportService,
    algorithmsService: {
      listAlgorithms: async () => [],
      applyAlgorithm: async () => ({ success: true, algorithm_name: "test", mutants_ranked: 0, message: "" }),
    } as AlgorithmsService,
    mutantsService: {
      listProjectMutants: async () => [],
      getMutant: async () => ({} as any),
      getMutantSourceCode: async () => ({ found: false, content: null, fullyQualifiedName: null }),
    } as MutantsService,
    ratingsService: {
...
export function makeMutant(overrides: Partial<MutantOverview> = {}): MutantOverview {
  return {
    id: 1,
    detected: true,
    status: "SURVIVED",
