...
vi.mock("@/components/dashboard/CreateProjectModal", () => ({
	CreateProjectModal: ({open}: {open: boolean}) => (open ? <div>CreateProjectModal</div> : null),
}));
...
describe("Dashboard", () => {
	...
	it("filters projects by search query", () => {
		renderWithProviders(<Dashboard />);
		fireEvent.change(screen.getByLabelText("Search projects"), {target: {value: "beta"}});
	...
	it("shows no-results state when search has no matches", () => {
		...
		expect(screen.getByText('No projects found for "missing". Try a different search term')).toBeInTheDocument();
	});
