...
describe("ExportServiceImpl", () => {
    ...
    it("getExportPreview calls get with project id", async () => {
        ...
        expect(apiClient.get).toHaveBeenCalledWith(
            "/admin/projects/1/export/preview",
            expect.any(Object),
        );
    });
    ...
    it("downloadExport calls downloadFileWithName", async () => {
        const {apiClient} = await import("@/api/client");
        (
            apiClient.downloadFileWithName as ReturnType<typeof vi.fn>
        ).mockResolvedValue(undefined);
        await service.downloadExport(1, "export.json");
        expect(apiClient.downloadFileWithName).toHaveBeenCalledWith(
            "/admin/projects/1/export",
            "export.json",
        );
    });
