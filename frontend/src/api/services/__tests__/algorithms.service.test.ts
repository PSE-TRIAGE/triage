...
describe("AlgorithmsServiceImpl", () => {
	...
	it("listAlgorithms calls get and extracts algorithms", async () => {
		...
		(apiClient.get as ReturnType<typeof vi.fn>).mockResolvedValue({
			algorithms: [{ id: "algo1", name: "Algorithm 1", description: "Test" }],
		});
		const result = await service.listAlgorithms();
		expect(apiClient.get).toHaveBeenCalledWith("/algorithms", expect.any(Object));
	...
	it("applyAlgorithm calls post with correct endpoint and data", async () => {
		...
		expect(apiClient.post).toHaveBeenCalledWith("/projects/5/algorithm", expect.any(Object), { algorithm: "algo1" });
