...
describe("RatingsServiceImpl", () => {
	...
	it("getRating calls get with mutant id", async () => {
		...
		expect(apiClient.get).toHaveBeenCalledWith("/mutants/10/ratings", expect.any(Object));
	...
	it("submitRating calls post with mutant id and data", async () => {
		...
		(apiClient.post as ReturnType<typeof vi.fn>).mockResolvedValue(ratingData);
		const result = await service.submitRating(10, { field_values: [] });
		expect(apiClient.post).toHaveBeenCalledWith("/mutants/10/ratings", expect.any(Object), { field_values: [] });
