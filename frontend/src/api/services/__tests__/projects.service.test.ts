...
describe("ProjectsServiceImpl", () => {
	...
	it("listProjects calls get", async () => {
		...
		expect(apiClient.get).toHaveBeenCalledWith("/projects", expect.any(Object));
	...
	it("deleteProject calls delete with correct endpoint", async () => {
		...
		expect(apiClient.delete).toHaveBeenCalledWith("/admin/projects/42", expect.any(Object));
	});

	it("renameProject calls patch with correct endpoint", async () => {
		const { apiClient } = await import("@/api/client");
		(apiClient.patch as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 1, name: "New Name" });
		const result = await service.renameProject(1, { name: "New Name" });
		expect(apiClient.patch).toHaveBeenCalledWith("/admin/projects/1/name", expect.any(Object), { name: "New Name" });
	...
	it("listAdminProjects calls get", async () => {
		...
		expect(apiClient.get).toHaveBeenCalledWith("/admin/projects", expect.any(Object));
	});
	...
	it("listProjectUsers calls get with project id", async () => {
		...
		expect(apiClient.get).toHaveBeenCalledWith("/admin/projects/5/users", expect.any(Object));
	});

	it("addUserToProject calls patch", async () => {
		const { apiClient } = await import("@/api/client");
		(apiClient.patch as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
		await service.addUserToProject(5, 3);
		expect(apiClient.patch).toHaveBeenCalledWith("/admin/projects/5/users/add/3", expect.any(Object));
	});
	...
	it("removeUserFromProject calls patch", async () => {
		...
		expect(apiClient.patch).toHaveBeenCalledWith("/admin/projects/5/users/remove/3", expect.any(Object));
	});

	it("createProject calls uploadFile", async () => {
		const { apiClient } = await import("@/api/client");
		(apiClient.uploadFile as ReturnType<typeof vi.fn>).mockResolvedValue({ id: 1, name: "Test" });
	...
	it("uploadSourceCode calls uploadFile with PUT", async () => {
		const { apiClient } = await import("@/api/client");
		(apiClient.uploadFile as ReturnType<typeof vi.fn>).mockResolvedValue({ detail: "ok" });
