...
describe("apiClient", () => {
	...
	it("POST sends JSON body", async () => {
		...
		const { apiClient } = await import("../client");
		const { z } = await import("zod");
		await apiClient.post("/test", z.object({ ok: z.boolean() }), { foo: "bar" });
	...
	it("uploadFile sends FormData", async () => {
		...
		await apiClient.uploadFile("/upload", z.object({ id: z.number() }), file, { key: "value" });
	...
	it("uploadFile supports PUT method", async () => {
		...
		const { z } = await import("zod");
		const file = new File(["content"], "test.xml", { type: "text/xml" });
		await apiClient.uploadFile("/upload", z.object({ id: z.number() }), file, undefined, "PUT");
