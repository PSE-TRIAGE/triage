...
describe("AlgorithmSettingsTab", () => {
	beforeEach(() => {
		mockUseAlgorithms.mockReturnValue({
			data: [
				{id: "algo1", name: "Random Sort", description: "Randomly sort mutants"},
				{id: "algo2", name: "Priority Sort", description: "Sort by priority"},
			],
			isLoading: false,
			error: null,
		});
	...
	it("shows loading state", () => {
		mockUseAlgorithms.mockReturnValue({data: [], isLoading: true, error: null});
		renderWithProviders(<AlgorithmSettingsTab />);
		expect(screen.getByText("Loading algorithms...")).toBeInTheDocument();
	});

	it("shows error state", () => {
		mockUseAlgorithms.mockReturnValue({data: [], isLoading: false, error: new Error("fail")});
		renderWithProviders(<AlgorithmSettingsTab />);
		expect(screen.getByText("Failed to load algorithms. Please try again.")).toBeInTheDocument();
	});

	it("shows empty state", () => {
		mockUseAlgorithms.mockReturnValue({data: [], isLoading: false, error: null});
		renderWithProviders(<AlgorithmSettingsTab />);
		expect(screen.getByText("No algorithms available yet.")).toBeInTheDocument();
	});

	it("renders description text", () => {
		renderWithProviders(<AlgorithmSettingsTab />);
		expect(screen.getByText(/Choose the mutation sorting algorithm/)).toBeInTheDocument();
	});
});
