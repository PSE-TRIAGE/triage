...
const mockMutants = [
	{id: 1, mutator: "ConditionalsBoundary", status: "SURVIVED", lineNumber: 10, rated: false, ranking: 1, detected: false, sourceFile: "A.java"},
	{id: 2, mutator: "NegateConditionals", status: "KILLED", lineNumber: 20, rated: true, ranking: 2, detected: true, sourceFile: "B.java"},
	{id: 3, mutator: "VoidMethod", status: "NO_COVERAGE", lineNumber: 30, rated: false, ranking: 3, detected: false, sourceFile: "C.java"},
];

describe("MutationListPanel", () => {
	beforeEach(() => {
		useMutantStore.setState({mutants: mockMutants as any[], selectedMutant: null});
	});
	...
	it("shows empty state when all mutants are reviewed", () => {
		...
		useMutantStore.setState({mutants: allReviewed as any[], selectedMutant: null});
		renderWithProviders(<MutationListPanel />);
		expect(screen.getByText(/No unreviewed mutants left/)).toBeInTheDocument();
	});
	...
	it("renders mutator text", () => {
		...
		expect(screen.getAllByText((_, el) => el?.textContent?.includes("ConditionalsBoundary") ?? false).length).toBeGreaterThan(0);
	});
});
