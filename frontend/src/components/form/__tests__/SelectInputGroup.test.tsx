...
describe("SelectInputGroup", () => {
	it("renders label", () => {
		render(<SelectInputGroup label="Choose option" onValueChange={vi.fn()} />);
		expect(screen.getByText("Choose option")).toBeInTheDocument();
	});

	it("shows required indicator when showRequired is true", () => {
		render(<SelectInputGroup label="Select" showRequired onValueChange={vi.fn()} />);
		expect(screen.getByText("*")).toBeInTheDocument();
	});

	it("renders error message", () => {
		render(<SelectInputGroup label="Select" error="Selection required" onValueChange={vi.fn()} />);
