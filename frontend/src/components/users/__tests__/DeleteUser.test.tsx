...
describe("DeleteUser", () => {
	...
	it("deactivates user by default when confirmed", async () => {
		...
		fireEvent.click(screen.getByRole("button", {name: "Yes, deactivate user"}));
	...
	it("deletes user data when checkbox is selected", async () => {
		...
		fireEvent.click(screen.getByRole("button", {name: "Yes, delete user data"}));
	...
	it("prevents self-deactivation and shows toast error", async () => {
		useMeMock.mockReturnValueOnce({data: {id: "1", username: "target_user"}});
		renderWithProviders(
			<DeleteUser user={mockUser as any} open={true} onOpenChange={vi.fn()} />,
		);
	...
	it("calls onOpenChange(false) when cancel is clicked", async () => {
		...
		renderWithProviders(
			<DeleteUser user={mockUser as any} open={true} onOpenChange={onOpenChange} />,
		);
